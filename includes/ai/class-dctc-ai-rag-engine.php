...
class DCTC_AI_RAG_Engine {
	...
	private function __construct() {
		...
		require_once DCTC_PLUGIN_DIR . 'includes/ai/class-dctc-ai-embedding-manager.php';

		$this->indexer             = new DCTC_AI_Indexer();
		$this->retriever           = new DCTC_AI_Retriever();
		$this->embedding_manager   = new DCTC_AI_Embedding_Manager();
	}
	...
	public function start_index_job( $post_types ) {
		...
		wp_schedule_single_event( time(), 'dctc_ai_rag_process_index_batch' );
	...
	public function process_index_batch_tick() {
		...
		if ( 'indexing' === $status ) {
			...
			if ( $result['remaining'] > 0 ) {
				wp_schedule_single_event( time() + 5, 'dctc_ai_rag_process_index_batch' );
				return;
			}
	...
	public function init() {
		...
		add_action( 'dctc_ai_rag_auto_index', [ $this, 'auto_index_documents' ] );
		add_action( 'dctc_ai_rag_process_embeddings', [ $this, 'process_pending_embeddings' ] );
		add_action( 'dctc_ai_rag_process_index_batch', [ $this, 'process_index_batch_tick' ] );
		add_action( 'dctc_ai_rag_reindex_post', [ $this, 'on_post_reindex' ], 10, 1 );
	...
	public function retrieve_context( $query, $limit = 5 ) {
		...
		try {
			...
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				error_log( 'Dragwyb AI AI RAG Engine Search Error: ' . $e->getMessage() ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Allowed under WP_DEBUG constraint.
			}
			return [];
		}
	}
	...
	public function on_post_status_change( $new_status, $old_status, $post ) {
		...
		if ( 'publish' === $new_status ) {
			wp_schedule_single_event( time(), 'dctc_ai_rag_reindex_post', [ $post->ID ] );
		}
	...
	public function on_plugin_change() {
		...
		wp_schedule_single_event( time() + 300, 'dctc_ai_rag_auto_index' );
	}
