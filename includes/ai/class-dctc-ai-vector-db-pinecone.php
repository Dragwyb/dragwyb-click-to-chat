<?php
/**
 * DCTC AI Vector DB - Pinecone Adapter
 *
...
require_once DCTC_PLUGIN_DIR . 'includes/ai/class-dctc-ai-vector-db-base.php';

/**
 * Class DCTC_AI_Vector_DB_Pinecone
 */
class DCTC_AI_Vector_DB_Pinecone extends DCTC_AI_Vector_DB_Base
{
	...
	public function __construct($config = [])
	{
		 ...
		 *
		 * Example:
		 * https://Dragwyb AI-index-xxxxx.svc.aped-4627-b74a.pinecone.io
		 */
	...
	public function test_connection($expected_dimensions = null)
	{
		...
		if ($status >= 200 && $status < 300) {
			...
			if (isset($data['dimension'])) {
				...
				$embedding_manager = new DCTC_AI_Embedding_Manager();
				...
				if ($dimensions && (int) $data['dimension'] !== $dimensions) {
					...
					return new WP_Error(
						...
						sprintf(
							...
							__('Pinecone index dimension mismatch! Your Pinecone index has %1$d dimensions, but your selected AI provider (%2$s) requires %3$d dimensions. Please recreate your Pinecone index with exactly %3$d dimensions.', 'dragwyb-click-to-chat'),
							(int) $data['dimension'],
							$provider_name,
							$dimensions
						)
		...
		if ($status === 401) {
			return new WP_Error(
				...
				__('Invalid Pinecone API Key. Please check your credentials and try again.', 'dragwyb-click-to-chat')
			);
		}
		...
		if ($status === 403) {
			return new WP_Error(
				'pinecone_forbidden', 
				__('Access to Pinecone index forbidden. Check if your API key has correct permissions.', 'dragwyb-click-to-chat')
			);
		}
		...
		if ($status === 404) {
			return new WP_Error(
				'pinecone_not_found', 
				__('Pinecone host or index not found. Please verify your Host URL.', 'dragwyb-click-to-chat')
			);
		}

		return new WP_Error(
			'pinecone_connection_failed',
			!empty($body) ? $body : __('Pinecone connection failed with status code: ', 'dragwyb-click-to-chat') . $status
		);
	}
